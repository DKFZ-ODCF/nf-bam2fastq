...
setUp_BashSucksVersion() {
  ...
  lockResource "tmpFiles"
  echo "$ARRAY_ELEMENT_DUMMY" > "$(tmpFilesFile)"
  unlockResource "tmpFiles"

  # Remove all registered temporary files upon exit.
  trap 'cleanUp_BashSucksVersion; rm "$(tmpFilesFile)"; rmdir "$(tmpDir)"' EXIT
}
cleanUp_BashSucksVersion() {
  lockResource "tmpFiles"
  declare -a tmpFiles=( $(tac "$(tmpFilesFile)") )
...
setUp() {
  ...
  lockResource "tmpFiles"
  cat /dev/null > "$(tmpFilesFile)"
  unlockResource "tmpFiles"

  # Remove all registered temporary files upon exit
  trap 'cleanUp; rm "$(tmpFilesFile)"; rmdir "$(tmpDir)"' EXIT
}
cleanUp() {
  lockResource "tmpFiles"
  declare -a tmpFiles=( $(tac "$(tmpFilesFile)") )
...
waitForFile() {
  ...
  local waitCount=0
  while [[ ${waitCount} -lt 3 && ! (-r "$file") ]]; do
    sleep 5
